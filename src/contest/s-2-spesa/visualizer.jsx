    ...
    <>
      <Canvas gravity="bottom" scale={10}>
        {range(state.N).flatMap((i) => (
          <Sprite key={i} src={state.queue[i] == 2 ? adulto : bambino} alt={state.queue[i] == 2 ? "adulto" : "bambino"} x={i >= state.pos ? 30 + (i-state.pos) * 9 : 17 + (i-state.pos)*6} y={4.8} />
        ))}
        {range(state.N).flatMap((i) => {
          return range(state.wealth[i]).map((j) => (
            <Sprite key={nc++} src={carrot} alt="carota" x={18.5 + state.wealth[i]/2 + (i-state.pos)*6 - j*0.7} y={5 + state.wealth[i] + j * 0.4} />
          ));
        })}
      ...
      <Variables variables={{ ...variables, N: state.N, K: state.K, "euro spesi": state.cost }} />
    </>
  );
}
