    ...
    <>
      <Canvas scale={130}>
        ...
        {range(state.N - Math.max(state.pos-1, 0)).map((x) => {
          let i = Math.max(state.pos-1, 0) + x;
          let pos = position(state.pos == 0 ? x : x-1, state.queue[i].endsWith(" cotta"));
          return <Sprite
            key={i}
        ...
        <Sprite src={state.pos < state.N && state.queue[state.pos].endsWith(" cotta") ? stoveOn : stoveOff} alt="Fornello" x={2.2} y={1.5} />
      </Canvas>
