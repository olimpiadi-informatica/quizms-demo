      ...
      <Canvas gravity="bottom" scale={10}>
        {range(state.N).map((i) => (
          <Rectangle
            ...
            y={6}>
          <div
            style={{ color: state.light[i] ? "black" : "white", textDecorationColor: state.light[i] ? "black" : "white" }}
        ...
        {range(state.N).map((i) => (
          <Sprite key={"bulb"+i} src={state.light[i] ? bulbOn : bulbOff} alt="" x={width*(i+1.5) - 1.5} y={20} />
        ))}
        {range(state.N*2).map((i) => (
          <Sprite key={"switch"+i} src={i > 0 && state.light[Math.floor((i-1)/2)] ? (state.switch[i] ? switchOnOn : switchOnOff) : (state.switch[i] ? switchOffOn : switchOffOff)} alt="" x={width*(0.5*i+0.75) - 1.5} y={10} />
        ))}
        {range(state.N).map((i) => (
          <Fragment key={"door"+i}>
            {(state.door[i] ? (
        ...
        <Sprite key="bunny" src={state.dir == 1 ? bunnyRight : bunnyLeft} alt="Bunny" x={width*(state.pos+0.5) - 3.5} y={4} follow />
      </Canvas>
      <Variables
        variables={{
          ...variables,
